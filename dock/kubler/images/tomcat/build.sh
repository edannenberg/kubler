...
finish_rootfs_build()
{
    ...
    tomcat_path="${_EMERGE_ROOT}"/usr/share/tomcat-9
    ...
    tomcat_deps="$(java-config --query DEPEND --package tomcat-9)"
    ...
    sed -i "s|CLASSPATH=\`java-config --classpath tomcat-9\`|CLASSPATH=`java-config --with-dependencies --classpath tomcat-9,tomcat-native`|g" "${catalina}"
